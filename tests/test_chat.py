...
class TestChat(unittest.TestCase):
    ...
    def test_system_prompt_in_messages(self, mock_print, mock_input, mock_stream_generate, mock_make_prompt_cache, mock_load):
        ...
        # Test with system prompt
        with patch('sys.argv', ['chat.py', '--system-prompt', 'You are a weather assistant.']):
            try:
                main()
            except SystemExit:
                pass
        ...
        call_args = mock_tokenizer.apply_chat_template.call_args[0][0]  # First positional arg (messages)
    ...
    @patch('builtins.input')
    @patch('builtins.print')
    def test_no_system_prompt_in_messages(self, mock_print, mock_input, mock_stream_generate, mock_make_prompt_cache, mock_load):
        from mlx_lm.chat import main
