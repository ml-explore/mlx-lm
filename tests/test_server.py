...
class TestServer(unittest.TestCase):
    ...
    def test_openresponses_tool_calling(self):
        if not self.response_generator.model_provider.tokenizer.has_tool_calling:
            self.skipTest("test model tokenizer does not support tool calling")
        response = requests.post(
            self._responses_url(),
            json={
                "model": "chat_model",
    ...
    def test_openresponses_compact_response(self):
        response = requests.post(
            self._responses_url("/v1/responses/compact"),
            json={
                "model": "chat_model",
