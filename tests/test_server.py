...
class TestChatCompletionContentKey(unittest.TestCase):
    """`content` is present on a non-streaming chat completion, even when empty.

    A model that stops while still inside a reasoning block returns no text, and
    the schema keeps `content` present and nullable, so a client reading
    `message["content"]` gets None rather than a KeyError.
    """

    def _handler(self, stream, object_type="chat.completion"):
        h = APIHandler.__new__(APIHandler)
        h.request_id = "chatcmpl-test"
        h.system_fingerprint = "fp-test"
        h.object_type = object_type
        h.requested_model = "test-model"
        h.created = 0
        h.stream = stream
        return h

    def test_content_present_when_only_reasoning(self):
        h = self._handler(stream=False)
        r = h.generate_response(
            "",
            "length",
            prompt_token_count=1,
            completion_token_count=1,
            reasoning_text="thinking but never finished",
        )
        message = r["choices"][0]["message"]
        self.assertIn("content", message)
        self.assertIsNone(message["content"])
        self.assertEqual(message["reasoning"], "thinking but never finished")

    def test_content_present_when_empty_and_no_reasoning(self):
        h = self._handler(stream=False)
        r = h.generate_response(
            "", "stop", prompt_token_count=1, completion_token_count=0
        )
        message = r["choices"][0]["message"]
        self.assertIn("content", message)
        self.assertIsNone(message["content"])

    def test_text_still_passes_through(self):
        h = self._handler(stream=False)
        r = h.generate_response(
            "hello", "stop", prompt_token_count=1, completion_token_count=1
        )
        self.assertEqual(r["choices"][0]["message"]["content"], "hello")

    def test_stream_delta_may_omit_content(self):
        # A delta carrying only a role is normal between chunks, so streaming
        # keeps the old behaviour.
        h = self._handler(stream=True, object_type="chat.completion.chunk")
        r = h.generate_response("", None)
        self.assertNotIn("content", r["choices"][0]["delta"])
