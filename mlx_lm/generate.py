...
def speculative_generate_step(
    ...
    *,
    ...
    Args:
        ...
        tokenizer: The verifier model's tokenizer. Required when
          ``draft_tokenizer`` is provided for cross-tokenizer translation.
        draft_tokenizer: The draft model's tokenizer. When provided (and
          differs from ``tokenizer``), draft tokens are decoded to text and
          re-encoded with the verifier tokenizer before verification.
    ...
    cross_tokenizer = draft_tokenizer is not None
    ...
    def _step(model, cache, y, n_predict=1, apply_logits_processors=True):
        with mx.stream(generation_stream):
            ...
            if apply_logits_processors and logits_processors:
    ...
    def _draft_generate(y, num_draft):
        ...
        for _ in range(num_draft):
            y, _ = _step(
                draft_model, draft_cache, y,
                apply_logits_processors=not cross_tokenizer,
            )
            mx.async_eval(y)
            ys.append(y)
    ...
    if cross_tokenizer:
        ...
        draft_prompt = mx.array(
            draft_tokenizer.encode(prompt_text, add_special_tokens=False),
            mx.uint32,
        )
        with mx.stream(generation_stream):
            draft_y = _prefill(draft_model, draft_cache, draft_prompt)
        ...
        try:
            while True:
                ...
                # Draft n tokens in draft token space
                ...
                if n_verifier == 0:
                    ...
                    if draft_corr:
                        ...
                        draft_y = mx.array(fallback[-1:], mx.uint32)
                    continue

                # Verify re-encoded tokens with the verifier model
                if prev_tokens is not None:
                    prev_tokens = prev_tokens[
                        : prev_tokens.size - y.size - n_verifier + 1
                    ]
                y_verify = mx.concatenate(
                    [y, mx.array(verifier_token_ids, mx.uint32)]
                )
                tokens_v, logprobs_v = _step(
                    model, model_cache, y_verify, n_verifier + 1
                )
                needs_cleanup = True
                mx.eval(tokens_v)
                ...
                for i in range(n_accept_v):
                    ntoks += 1
                    yield verifier_token_ids[i], logprobs_v[i], True
                    if ntoks == max_tokens:
                        break

                # Yield the verifier's correction token
                if ntoks < max_tokens:
                    ntoks += 1
                    yield tokens_list[n_accept_v], logprobs_v[n_accept_v], False
                ...
                # Rewind draft cache: keep only the draft_y_old entry from
                # this round so we can replay the accepted text
                cache.trim_prompt_cache(draft_cache, num_draft - 1)
                needs_cleanup = False
                ...
                y = mx.array([correction_token], mx.uint32)

                if prev_tokens is not None:
                    prev_tokens = prev_tokens[: -max(n_verifier - n_accept_v, 1)]
        ...
        # Original same-tokenizer speculative decoding
        with mx.stream(generation_stream):
    ...
    **kwargs,
    ...
    Args:
        ...
        draft_model (Optional[nn.Module]): An optional draft model. If provided
          then speculative decoding is used. Default: ``None``.
        draft_tokenizer: The draft model's tokenizer. When provided, enables
