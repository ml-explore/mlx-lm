...
class GenerationResponse:
    ...
    Args:
        ...
        logprobs (mx.array, optional): A vector of log probabilities, or
          ``None`` when generation runs with ``compute_logprobs=False``.
...
def generate_step(
    ...
    *,
    ...
    Args:
        ...
        compute_logprobs (bool, optional): Normalize and yield log probabilities.
          If ``False``, the sampler runs on the raw logits (identical result for
          all built-in samplers, which are shift-invariant) and ``None`` is
          yielded instead of the log-probability vector. Default: ``True``.

    Yields:
        Tuple[mx.array, mx.array]: One token and a vector of log probabilities
          (``None`` when ``compute_logprobs=False``).
    """
    ...
    def _step(input_tokens: mx.array, input_embeddings: Optional[mx.array] = None):
        ...
        with mx.stream(generation_stream):
            ...
            # All built-in samplers are shift-invariant, so sampling from the
            # raw logits is identical to sampling from normalized logprobs.
    ...
    mx.async_eval(*[x for x in (y, logprobs) if x is not None])
    n = 0
    while True:
        if n != max_tokens:
            next_y, next_logprobs = _step(y)
            mx.async_eval(*[x for x in (next_y, next_logprobs) if x is not None])
        if n == 0:
            mx.eval(y)
...
def generate(
    ...
    **kwargs,
    ...
    """
    # generate() only consumes text; skip the logprob normalization by default.
    kwargs.setdefault("compute_logprobs", False)
    if verbose:
        print("=" * 10)
