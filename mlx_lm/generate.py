...
def speculative_generate_step(
    ...
    *,
    ...
    Note:
        The accept/reject test above compares each draft token against the
        model's own greedy (argmax) choice, so this generator is lossless
        with respect to plain autoregressive generation *in exact
        arithmetic*. In practice, with a greedy sampler (``temp=0``) it can
        still occasionally emit a different token than ``generate_step``
        would on the same prompt. This happens when two candidate tokens'
        logits are exactly tied at the model's compute precision (common in
        ``bfloat16``, which has only an 8-bit mantissa): the draft's target
        scoring runs as one batched forward pass over the whole draft block,
        while plain generation scores sequentially one token at a time.
        These are mathematically equivalent but take different reduction
        orders through matmul/attention, so a tied value can round
        differently and break the tie in favor of a different token in each
        path. This is expected floating-point non-associativity, not a bug
        in the accept/reject/rewind logic, and it does not occur when the
        top logits aren't exactly tied.
