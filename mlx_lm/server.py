    ...
    def _tokenize(self, tokenizer, request, args):
        if request.request_type == "chat":
            ...
            if tokenizer.has_chat_template:
                ...
                if self.model_provider.cli_args.trust_client_kwargs:
    ...
    def generate_response(
        ...
        if top_logprobs:
            response["choices"][0]["logprobs"] = {"content":
                [dict(i[0], top_logprobs=i) for i in top_logprobs]
            }
        elif token_logprobs:
            response["choices"][0]["logprobs"] = {"content":
...
def main():
    ...
    parser.add_argument(
        "--trust-client-kwargs",
        action="store_true",
        help="Enable chat_template_kwargs, allowing clients to send arguments to apply_chat_template.",
    )
    parser.add_argument(
        "--decode-concurrency",
        type=int,
