...
from .server_utils import RequestMetrics, ServerStats
...
class ResponseGenerator:
    def __init__(self, model_provider: ModelProvider, prompt_cache: LRUPromptCache):
        ...
        self.requests = Queue()
        self.stats = ServerStats(pending_counter=lambda: self.requests.qsize())
        ...
        while not self._stop:
            ...
            # No request so serve from the current batch
                ...
                while True:
                    ...
                    self.stats.add_tokens(generation_tokens=len(responses))
    ...
    def _serve_single(self, request):
        ...
        try:
            ...
            for gen in stream_generate(
                ...
                self.stats.add_tokens(generation_tokens=1)
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def handle_completion(self, request: CompletionRequest, stop_words: List[str]):
        ...
        metrics = RequestMetrics(self.response_generator.stats, self.request_id)

        # Create keepalive callback to send SSE comments during long prompt processing
        def keepalive_callback(processed_tokens, total_tokens):
            metrics.prompt_progress(processed_tokens, total_tokens)
            if self.stream:
                try:
        ...
        metrics.start_generation(len(ctx.prompt))
