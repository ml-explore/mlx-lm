...
class Attention(nn.Module):
    def __init__(self, config: TextConfig, layer_idx: int):
        ...
        # For full attention with k_eq_v, use global_head_dim for KV heads
    ...
    def __call__(
        ...
        offset = (cache.offset + 0) if cache is not None else 0
...
class Model(nn.Module):
    ...
    def sanitize(self, weights):
        # Normalize weight keys from various formats:
        #   HF:  model.language_model.model.layers.0... -> language_model.layers.0...
        #   MLX: language_model.model.layers.0...       -> language_model.layers.0...
        result = {}
        skip = {"vision_tower", "audio_tower", "embed_vision", "embed_audio", "multi_modal_projector"}
        for k, v in weights.items():
            if any(s in k for s in skip):
                continue
            # Strip outer "model." wrapper (HF format)
            if k.startswith("model."):
                k = k[len("model."):]
            # Strip inner "model." between language_model and layers/embed/norm
            k = k.replace("language_model.model.", "language_model.")
            result[k] = v
