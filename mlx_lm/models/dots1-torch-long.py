...
class Dots1Attention(nn.Module):
    def __init__(self, config: Dots1Config, layer_idx: int):
        ...
        self.q_norm = nn.RMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = nn.RMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
