...
def chunk_simple_gla(q: mx.array, k: mx.array, v: mx.array, g: mx.array, initial_state: mx.array) -> mx.array:
    return initial_state


def fused_recurrent_simple_gla(q: mx.array, k: mx.array, v: mx.array, g: mx.array, initial_state: mx.array) -> mx.array:
    return initial_state
...
class BailingMoeLinearLinearAttention(nn.Module):
    def __init__(self, args: ModelArgs, layer_idx: int):
        ...
        self.g_norm = BailingMoeLinearV2GroupRMSNorm(args.num_attention_heads * self.head_dim, eps=args.rms_norm_eps, group=args.group_norm_size)

        if args.use_qk_norm:
            self.key_layernorm = nn.RMSNorm(self.head_dim, eps=args.rms_norm_eps)
            self.query_layernorm = nn.RMSNorm(self.head_dim, eps=args.rms_norm_eps)
        
        self.slope = - self.make_slope(args.num_attention_heads) * (1 - (self.layer_idx - 1) / (args.num_hidden_layers - 1) + 1e-5)
    ...
    def __call__(
        ...
        if cache is not None:
            ...
            keys = self.rope(keys, offset=cache.offset)
            keys, values = cache.update_and_fetch(keys, values)
            recurrent_state = None # get from mamba cache
        else:
            queries = self.rope(queries)
            keys = self.rope(keys)
        ...
        if mode == "fused_recurrent":
            output = fused_recurrent_simple_gla(
                ...
                k=keys,
                v=values,
                g=self.slope[None, None, :].reshape(B, L, self.num_attention_heads),
...
class BailingMoeLinearDecoderLayer(nn.Module):
    def __init__(self, args: ModelArgs, layer_idx: int):
        ...
        if self.attention_layer_type == "attention":
            ...
            self.attention = BailingMoeLinearLinearAttention(
                args=args,
                layer_idx=layer_idx
            )
