...
class Indexer(nn.Module):
    ...
    def __call__(
        ...
        if cache is not None:
            # Indexer only needs keys; pass zero-width values to reuse KVCache
...
class GlmMoeDsaAttention(nn.Module):
    ...
    def __call__(
        ...
        if cache is not None:
            ...
            # Create placeholder so cache[0] and cache[1] are indexable below
            cache = [None] * 2
...
class GlmMoeDsaDecoderLayer(nn.Module):
    def __init__(self, config: ModelArgs, layer_idx: int):
        ...
        self.mlp = GlmMoeDsaMoE(config) if use_moe else GlmMoeDsaMLP(config)
...
class GlmMoeDsaModel(PipelineMixin, nn.Module):
    def __init__(self, config: ModelArgs):
        super().__init__()
    ...
    def __call__(
        ...
        # B1 fix: index into CacheList for mask creation
        ...
        # B1 fix: use cache[-1][0].keys for CacheList-aware depends
        if pipeline_rank != 0:
...
class Model(nn.Module):
    ...
    def sanitize(self, weights):
        ...
        # Remove rotary_emb.inv_freq keys
