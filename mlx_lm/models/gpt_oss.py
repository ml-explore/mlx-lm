...
class GptOssMoeModel(nn.Module):
    ...
    def __call__(
        ...
        if mask is None:
            sa = self.layer_types.index("sliding_attention")
            ga = self.layer_types.index("full_attention")
            full_mask = create_attention_mask(x, cache[ga : ga + 1])
            sliding_window_mask = create_attention_mask(x, cache[sa : sa + 1])

        for layer, c, layer_type in zip(self.layers, cache, self.layer_types):
            local_mask = mask
            if mask is None and layer_type == "full_attention":
